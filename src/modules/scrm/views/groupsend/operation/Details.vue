    ...
    <div class="main-content">
      <el-form :model="message" ref="message" label-width="auto" class="demo-message">
        ...
        <el-form-item label="" v-if="enclosureType == 'image_text'">
          <el-form style="margin-left: -65px" ref="imageText" :model="imageText" label-width="auto">
            <el-form-item label="链接标题">
              <el-input maxlength="64" v-model="imageText.name" placeholder="请输入链接标题"></el-input>
            </el-form-item>
        ...
        <!-- 小程序 -->
        <el-form-item style="margin-left: -70px" label="" v-if="enclosureType == 'program'">
          <select-program-item
        ...
        <el-form-item v-if="enclosureType !== 'text' && enclosureType !== 'image' && enclosureType !== 'program'">
...
import MaterialBtn from '@/modules/scrm/components/material/MaterialBtn.vue'
...
export default {
  ...
  data() {
    return {
      ...
      // zhw
      isType: 'image',
      webClone: false, //是否复制网页的信息
      isWebList: [] // 存储选择素材管理后网页的信息
      // zhw end
    }
  },
  methods: {
    ...
    materialSuccess(val) {
      ...
      if (val.type === 'image') {
        ...
        //this.imageUrl 是配合显示右边手机模型的图片
        this.imageUrl = val.imageUrl
        console.log(this.imageUrl)
        // console.log('从素材库中选择的图片', this.imageUrl)
      }
      if (val.type == 'image_text' || val.type == 'numerous_image_text') {
        this.webClone = true
        this.isWebList = val
        setTimeout(() => {
          this.enclosureType = 'image_text'
        }, 100)
        this.imageText = {
          name: val.name,
          content: val.content,
      ...
      if (val.type == 'program') {
        ...
        console.log('选择素材库中的小程序', this.program)
      }
    },
      ...
      if (val != 'image_text') {
        ...
        if (this.webClone) {
          this.imageText = {
            ...
            content: this.isWebList.content,
            link: this.isWebList.link,
            imageUrl: this.isWebList.imageUrl,
            type: 'image_text'
          }
        }
        if (this.imageText.imageUrl == '') {
          this.imageTextFileList = []
        } else {
          if (this.imageText.imageUrl.includes('https://athena-1255600302.cos.ap-guangzhou.myqcloud.com')) {
            this.imageTextFileList[0] = {
              url: this.imageText.imageUrl,
              status: 'success'
            }
          } else {
            this.imageTextFileList[0] = {
              url: 'https://athena-1255600302.cos.ap-guangzhou.myqcloud.com' + this.imageText.imageUrl,
              status: 'success'
            }
          }
        }
    ...
    selectProgramItemChange(val) {
      this.program = val
      console.log('选择了小程序后>>>', this.program)
    },
    // 发布群发/保存草稿
    publishGroupPost(status) {
      ...
      if (this.enclosureType == 'image_text') {
        console.log('选择了图文之后', this.imageText)
        if (
          this.imageText.name == '' ||
      ...
      if (this.releaseTimeType == 'time' && (this.releaseTime == '' || this.releaseTime == null ) && status != null) {
      ...
      // if (this.noMessageDays != 0) {
      this.message.notReceiveGroupSendDays = this.noMessageDays
      // }
      console.log('提交的参数', this.message)
      this.isCanSubmit = true

      if (this.message.messageTargetList.length == 0) {
        this.$message.warning('选择的客户人数不能为0')
      ...
      this.$http[name]({ id: this.pageId }).then(res => {
        ...
        if (customerCondition.tagIdList || customerCondition.lifeCycleId) {
          ...
          this.companyTagsValue = customerCondition.tagIdList
            ...
            : []
          this.noMessageDays = customerCondition.notReceiveGroupSendDays
          this.selectForGroupSendChange()
        }
        ...
        if (this.enclosureType == 'image_text') {
          this.isType = this.enclosureType
        ...
        this.releaseTimeType = data.sendType
        this.releaseTime = data.scheduledSendTime
        this.message = data
      })
    },
    // 修改群发
