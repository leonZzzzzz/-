  ...
  >
    ...
    >
      ...
      <el-form-item
        ...
        :rules="[{ required: true, message: '该字段不能为空' }]"
      > 
        <div class="video-add" v-if="model.videoUrl == ''">
          <video-upload :showimage="hide" @onSuccess="getUploadVideo"></video-upload>
        </div>
        <span v-if="model.videoUrl!=''" style="width:200px">{{model.content}}</span>
        <el-button
          v-if="model.videoUrl!=''"
          type="primary"
          @click="model.videoUrl=''"
...
export default {
  ...
  data() {
    return {
      picurl: 'https://athena-1255600302.cosgz.myqcloud.com',
      hide: 'videocate',
      model: {
        name: '',
        groupId: '',
  ...
  methods: {
    ...
    getCarousel() {
      this.$http.materialDetail({ id: this.id }).then(res => {
        ...
        this.playerOptions.poster = res.data.data.imageUrl
    ...
    getUploadVideo(e) {
      console.log(e)
      this.model.content = e.file.name
      this.model.videoUrl = e.video.videoUrl
      this.model.fileId = e.video.fileId
    ...
    success() {
      ...
      //   })
      // }
      this.model.radarType='video'
      this.model.radarTitle=this.model.name
      this.model.link=this.model.videoUrl
