...
export default {
  ...
  data() {
    return {
      ...
      conversationQuantity: [], //聊天总数
      messageQuantity: [], //消息总数
      replyPercentage: [], //已回复聊天占比
      avgFirstReplySpeed: [], //平均回复时长
      dateTime: [],
      department: [], // 部门数据
      pickerMinDate: '',
      pickerOptions: {
      ...
      tableColumn: [
        ...
        {
          prop: 'conversationQuantity',
          title: '聊天总数',
          align: 'center'
        },
        {
          prop: 'messageQuantity',
          title: '消息总数',
          align: 'center'
        },
        {
          ...
          align: 'center',
          formatter: row => {
            return row.replyPercentage ? row.replyPercentage.toFixed(2) + '%' : '0%'
          }
        },
        {
          prop: 'avgFirstReplySpeed',
          title: '平均回复时长',
          align: 'center'
        }
      ]
  ...
  methods: {
        ...
        series: [
          {
            ...
            stack: '聊天总数',
            data: this.conversationQuantity,
            symbolSize: 10
          },
          {
            name: '消息总数',
          ...
          },
          {
            ...
            areaStyle: {
              color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
                {
                  offset: 0,
                  color: 'RGB(72,202,147,.1)'
                },
                {
                  offset: 1,
                  color: 'RGB(72,202,147,0)'
                }
              ])
            },
            stack: '已回复聊天占比',
            data: this.replyPercentage,
            symbolSize: 10
          },
          {
            name: '平均回复时长',
    ...
    customerGrowthList(options) {
      ...
      this.conversationQuantity = []
      this.messageQuantity = []
      this.replyPercentage = []
      this.avgFirstReplySpeed = []
      this.dateTime = []
      this.$http.conversationGrowthList(options).then(res => {
        let data = res.data.data
        this.tableData = data
        data.forEach(item => {
          this.conversationQuantity.push(item.conversationQuantity)
          this.messageQuantity.push(item.messageQuantity.toFixed(2))
          this.replyPercentage.push(item.replyPercentage)
          this.avgFirstReplySpeed.push(item.avgFirstReplySpeed)
          this.dateTime.push(`${Math.abs(item.day.split('-')[1])}/${Math.abs(item.day.split('-')[2])}`)
        })
        console.log(this.tableData)
        console.log(this.avgFirstReplySpeed)
        this.drawChart()
      })
    },
    // 获取成员数据
