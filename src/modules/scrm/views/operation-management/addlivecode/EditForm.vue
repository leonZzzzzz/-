        ...
        <div class="modular">
          ...
          <div class="web-div" v-if="model.materialList[0].type === 'image_text'" v-show="model.sendWelcomeType == 1">
            ...
            <el-form-item :required="true" label="封面图">
              <!-- <QcImageUpload width="145px" height="145px" v-model="linkImageUrl"> </QcImageUpload> -->
              <!-- <span>上传图片</span> -->
...
export default {
  ...
  methods: {
    ...
    addCode() {
      ...
      api.updateLiveCode(this.model).then(res => {
    ...
    getLiveCode() {
      ...
      api.getLiveCode({ id: this.routeId }).then(res => {
        ...
        this.isType = this.model.materialList[0].type
        if (this.model.materialList[0].type == 'image') {
          this.imgFileList[0] = {
            url: 'https://athena-1255600302.cos.ap-guangzhou.myqcloud.com' + this.model.materialList[0].imageUrl,
            status: 'success'
          }
          ...
          this.linkImageUrl = ''
          this.imageUrl = ''
          this.materialProgram.url =
            'https://athena-1255600302.cos.ap-guangzhou.myqcloud.com' + this.model.materialList[0].imageUrl
          this.materialProgram.name = this.model.materialList[0].name
          this.programModel.link = this.model.materialList[0].link
          this.programModel.name = this.model.materialList[0].name
          this.programModel.imageUrl = this.model.materialList[0].imageUrl
          this.model.materialList[0].name = ''
          this.model.materialList[0].link = ''
          this.model.materialList[0].imageUrl = ''
          ...
          this.lotteryDialog.selectedLotteryData = {
            title: this.model.materialList[0].name,
            id: this.model.materialList[0].source
          }
          this.model.materialList[0].source = ''
          this.model.materialList[0].name = ''
          this.model.materialList[0].programId = ''
          this.linkImageUrl = ''
          this.imageUrl = ''
        ...
        //如果有修改的头像就返回
        if (this.model.customizedLogo) {
          this.imgLogoList = [
            {
              url: this.imgHost + this.model.customizedLogo,
              name: this.model.name
            }
          ]
        }

        console.log(this.imgLogoList)
        console.log(this.model)
      })
    },
    closeAdd() {
