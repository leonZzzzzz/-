...
export default {
  ...
  methods: {
          ...
          },
          {
            ...
            areaStyle: {
              color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
                {
                  offset: 0,
                  color: 'RGB(85,225,223,.1)'
                },
                {
                  offset: 1,
                  color: 'RGB(85,225,223,0)'
                }
              ])
            },
            stack: '入群数',
          ...
          },
          {
            ...
            areaStyle: {
              color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
                {
                  offset: 0,
                  color: 'RGB(249,122,165,.1)'
                },
                {
                  offset: 1,
                  color: 'RGB(249,122,165,0)'
                }
              ])
            },
            stack: '退群数',
    ...
    groupGrowthList(options) {
      ...
      this.quit = []
      this.dateTime = []
      this.$http.groupGrowthList(options).then((res) => {
        let data = res.data.data
        this.tableData = data
        data.forEach((item) => {
          this.total.push(item.total)
          this.join.push(item.join)
          this.quit.push(item.quit)
    ...
    exportData() {
      ...
      this.$confirm('是否导出当前列表', '操作提示', {
        ...
        window.open(this.baseUrl + '/api/admin/v1/group-growth/export?' + qs.stringify(options))
      })
    }
  }
}
