  ...
  <div class="wrap" ref="wrap">
    <div ref="search">
      <button-wrap>
        ...
        <!-- <el-button type="primary" size="small" :disabled="tableList.id == null" @click="toMember()">查看参与嘉宾</el-button> -->
    ...
    <QcTableOld ref="table" :table-list="showData" :search="searchData" url="api/admin/v1/signIn/page"></QcTableOld>
    <el-dialog
      title="抽奖链接"
...
export default {
  ...
  methods: {
    async getMobileUrl() {
      ...
      this.shareHost = `${res.data.data.url}/plugin/#/signIn?`
    },
    //添加
    insert() {
      this.$router.push(`signIn/release`)
    },
    //修改
    update() {
      this.$router.push(`signIn/release?id=${this.tableList.id}`)
    },
    toMember() {
      this.$router.push(
