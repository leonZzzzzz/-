<template>
  <div class="newlyaddedDiv">
    <div class="top_nav">
      <el-button type="text" icon="el-icon-arrow-left" @click="$router.go(-1)">再次群发</el-button>
    </div>
          ...
          <div class="personal-send-form">
            <el-form label-width="120px">
              ...
              <el-form-item label="选择客户:" :required="true" v-if="models.chatType == 'single'">
                ...
                >
                  <el-alert
                    :title="`将群发消息给全部账号的${allCustomerListLength}个客户`"
                    type="warning"
                    show-icon
                    :closable="false"
                    style="height: 36px"
          ...
          <div class="personal-send-form">
            <el-form label-width="120px">
              ...
              <el-form-item class="btn-primary-padding">
                ...
                <!-- <el-button @click="editButton" type="info">修改</el-button> -->
              </el-form-item>
            </el-form>
          </div>
        </div>
    ...
    <com-dialog :config="addDialog.config" @closeDialog="closeAddDialog">
      <div class="add-content">
        ...
        <div class="footer_btns" style="display: flex; justify-content: flex-end; margin-top: 10px">
          <el-button @click="closeAddDialog" plain>取消</el-button>
    ...
    <com-dialog :config="changeGroupDialog.config" @closeDialog="closeGroupDialog">
      <div class="add-content">
        <div class="query-table">
          <div class="query-table-left" style="width: 100%; display: flex">
            <div class="search-input" style="margin-right: auto">
        ...
        <div v-for="(item, index) in tagEditNameList" :key="index">
          ...
          <div class="tagChange">
            <div
              ...
              v-for="(items, indexs) in item.tagList"
              :key="indexs"
              :class="{ active: spanEditIndex.indexOf(items.id) > -1 }"
            >
              {{ items.name }}
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    NewAddgetPage() {
      this.models.messageItemList = []
      api.reCordGroup({ id: this.$route.query.id }).then((res) => {
        ...
        if (this.models.chatType === 'single') {
          ...
          this.models.messageItemList.forEach((item) => {
            ...
            if (this.contentType == 'image') {
              ...
              console.log('aaaaaaaaaaaaaaaaaaaaa', item.programId)
              this.programIdProps.id = item.programId
              this.programIdProps.name = item.name
              this.programIdProps.logo = item.imageUrl
              // this.programIdProps = item.programId
              this.appProgramId = item.programId
              this.addProgramName = item.name
              this.addProgramLink = item.link
              this.addProgramImage = item.imageUrl
    ...
    editNum() {
      api.groupChatSendGet({ id: this.$route.query.id }).then((res) => {
        this.editModels = res.data.data
        this.editModels.name = this.models.name
        this.editModels.message = this.models.message
        this.editModels.messageItemList = this.models.messageItemList
        this.editModels.messageTargetList = this.models.messageTargetList
        api.groupChatUpdate(this.editModels).then(() => {
