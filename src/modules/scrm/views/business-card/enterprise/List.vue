      ...
      <div class="en-div">
        ...
        <el-form label-position="left" label-width="100px" v-if="isShow">
          ...
          <el-form-item label="官网简介:">
            <div class="introduct" v-html="tableData.introduction"></div>
          </el-form-item>
          ...
          <div class="btn-div">
            ...
            <span style="font-size: 14px; color: #606266; margin-left: 10px">(上传后将全部覆盖已上传的相册内容)</span>
          </div>
          <!-- <el-form-item label="视频封面" style="margin-top:20px">
            <img class="video-cover" :src="tableData.videoCover" v-if="tableData.videoCover != null" />
            <span v-else>暂无封面</span>
          </el-form-item> -->
          <el-form-item label="企业视频" style="margin-top: 20px">
            <video style="width: 30%" controls id="video" v-if="tableData.videoUrl">
              <source :src="tableData.videoUrl" type="video/mp4" />
            </video>
      ...
      <com-dialog :config="addDialog.config" @closeDialog="addDialog.config.visible = false">
        <el-form label-position="right" label-width="80px" class="dialog-div" v-if="addDialog.config.visible">
          ...
          <el-form-item label="企业视频">
            <UploadVideo :fileList="videoSuccessList" @success="SuccessVideoUpload"></UploadVideo>
          </el-form-item>
          <el-form-item label="企业标签" :required="true">
            ...
            <div v-if="addTable.tags && addTable.tags.length < 6">
      ...
      <com-dialog :config="editDialog.config" @closeDialog="editDialog.config.visible = false">
        <el-form label-position="right" label-width="80px" class="dialog-div">
          ...
          <el-form-item label="企业地址">
            ...
            <el-checkbox type="primary" style="margin-top: 10px" v-model="editTable.synchronizeBusinessCard"
              >是否同步更新成员名片</el-checkbox
            >
            <div style="margin-left: 20px; color: #606266; font-size: 12px; margin-top: -10px">
              &nbsp;(如勾选，所有成员名片中的地址将会一并更新为企业地址)
            </div>
          </el-form-item>
          <el-form-item label="企业电话">
          ...
          <el-form-item label="企业标签" :required="true">
            ...
            <div v-if="editTable.tags && editTable.tags.length < 6">
...
import UploadImageTwo from '../../../components/components/UploadImageTwo'
...
import UploadVideo from '../../../components/components/UploadVideoNoCover'
export default {
  components: { Tag, ComDialog, UploadImage, GoBack, UploadVideo, UploadImageTwo },
  data() {
    return {
      FristList: [],
      SecondList: [],
  ...
  methods: {
    ...
    SuccessVideoUpload(val) {
      if (val) {
        this.addTable.videoCover = val.coverUrl
        this.addTable.videoId = val.fileId
        this.addTable.videoUrl = val.videoUrl
      }
    },
    //修改企业信息上传视频成功返回的回调函数
    videoSuccess(val) {
      if (val) {
        this.editTable.videoCover = val.coverUrl
        this.editTable.videoId = val.fileId
        this.editTable.videoUrl = val.videoUrl
      }
    },
    //点击确认上传ppt
    sumbitPpt() {
    ...
    addaddEnterpriseKeyWord() {
      ...
      if (this.addTable.tags.length >= 6) {
        this.$message.warning('标签个数不能超过六个')
        return
      }
    ...
    addEnterpriseKeyWord() {
      ...
      if (this.editTable.tags.length >= 6) {
        this.$message.warning('标签个数不能超过六个')
        return
      }
    ...
    editMessage() {
      ...
      this.$set(this.editTable, 'synchronizeBusinessCard', false)
    ...
    editSumbit() {
      ...
      if (this.editTable.tags.length != 0) {
        ...
        if (flags == true) {
          ...
          if (this.editTable.logo.includes('https://athena-1255600302.cos.ap-guangzhou.myqcloud.com')) {
          ...
          if (this.editTable.cover.includes('https://athena-1255600302.cos.ap-guangzhou.myqcloud.com')) {
            this.editTable.cover = this.editTable.cover.replace(
              /https:\/\/athena-1255600302.cos.ap-guangzhou.myqcloud.com/,
              ''
            )
          }
          for (let i = 0; i < this.imageCoverFileList.length; i++) {
            if (this.imageCoverFileList[i].url.indexOf('blob') > -1) {
              this.$message.warning('请等待全部图片上传成功在进行保存')
              return
            }
          }
    ...
    getCropCompant() {
      this.$http.getCropCompant().then(res => {
        if (!res.data.data) {
          ...
          if (this.tableData.videoCover != null) {
            this.videoFileList.coverUrl = this.tableData.videoCover
          }
          if (this.tableData.videoId != null) {
            this.videoFileList.fileId = this.tableData.videoId
          }
          if (this.tableData.videoUrl != null) {
            this.videoFileList.videoUrl = this.tableData.videoUrl
          }
        }
      })
    }
