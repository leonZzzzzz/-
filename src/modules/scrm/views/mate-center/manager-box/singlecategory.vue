<template>
  ...
  >
    <div style="margin:10px 20px 20px 20px;">
      <el-radio-group v-model="radio" @change="checkRadio" style="margin-right:20px;">
    ...
    <el-form v-if="radio==1" :model="model" ref="model" label-width="90px" label-position="right">
      <el-form-item label="名称" prop="singleContent.title" :rules="[{ required: true, message: '该字段不能为空' }]">
        <el-input v-model="model.singleContent.title" style="width: 400px"></el-input>
      </el-form-item>
      ...
      <el-form-item label="简介" prop="singleContent.info">
        <el-input style="width: 400px" type="textarea" v-model="model.singleContent.info"></el-input>
      </el-form-item>
      ...
      <el-form-item label="雷达追踪">
        ...
        <div v-if="model.isRadar">
          <div class="setting-item">
              <el-checkbox v-model="model.radarRequest.isBehaviorNotice">行为通知</el-checkbox>
              <span class="s-tips">（当客户点击雷达时，发送雷达的员工将会收到消息提醒）</span>
            </div>
            <div class="setting-item">
              <el-checkbox v-model="model.radarRequest.isActionNotice">动态通知</el-checkbox>
              <span class="s-tips">（当客户点击雷达时，会将客户的打开行为记录在客户动态里）</span>
            </div>
            <div class="setting-item">
              <el-checkbox v-model="model.radarRequest.isBehaviorTag">客户标签</el-checkbox>
            ...
            <div class="setting-tags" v-if="model.radarRequest.isBehaviorTag">
              ...
              <div class="selected-tags-box" v-if="selectedBehaviorTags.length > 0">
                <div class="s-tag-item" v-for="(item, index) in selectedBehaviorTags" :key="index">
                  ...
                  分值&nbsp;&nbsp;<el-input-number size="mini" type="number" :min='1' v-model="item.businessScore"></el-input-number>
    ...
    <el-form v-if="radio==2" :model="model" ref="model" label-width="90px" label-position="right">
      ...
      <el-form-item label="链接" prop="link" :rules="[{ required: true, message: '该字段不能为空' }]">
        <el-input v-model="model.link" style="width: 400px"></el-input>
  ...
  methods: {
    ...
    getCarousel() {
      ...
      this.$http.materialDetail({ id: this.id }).then((res) => {
        if(res.data.data.mark=='link'){
          ...
          // 获取图文详情
          this.$http.MaterSingleDetail({id:this.singlefileId}).then(json=>{
    ...
    getRadarDetail(model,singleContent){
      ...
      this.$http.getInteractRadarDetail({id:model.radarId}).then(rep=>{
        ...
        this.model=model
        this.selectedBehaviorTags=rep.data.data.behaviorTagList
      })
    },
    // 获取行为标签
    getBehavoirTag(){
