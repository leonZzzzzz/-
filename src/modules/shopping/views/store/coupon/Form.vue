    ...
    saveModel(type, model) {
      couponApi[type](model)
        .then(() => {
          this.loading = false
          this.$message({
            message: ['updateCouponPublished', 'updateStoreCoupon'].includes(type) ? '修改成功' : '新建成功',
            type: 'success'
          })
          setTimeout(() => {
            this.$router.go(-1)
