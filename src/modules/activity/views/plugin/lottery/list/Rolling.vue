...
export default {
  ...
  methods: {
    ...
    //添加
    insert() {
      this.$router.push(`/main/activity/plugin/lottery/releaseQun?showType=1`)
    },
    //修改
    update(count) {
      if (count === 2) {
        this.$router.push(`/main/activity/plugin/lottery/releaseQun?id=${this.tableRow.id}&showType=1&copy=true`)
      } else {
        this.$router.push(`/main/activity/plugin/lottery/releaseQun?id=${this.tableRow.id}&showType=1`)
      }
    },
    jumpPage(type) {
      switch (type) {
        case 1:
          // 参与嘉宾
          this.$router.push(
            `/main/activity/plugin/lottery/member?id=${this.tableRow.id}&sourceId=${this.tableRow.sourceId}&sourceType=${this.tableRow.sourceType}&showType=1`
          )
          break
        case 2:
          // 中奖名单
          this.$router.push(
            `/main/activity/plugin/lottery/winning?id=${this.tableRow.id}&sourceType=${this.tableRow.sourceType}`
          )
          break
        case 3:
          // 查看弹幕
    ...
    open() {
      let path = '/lottery/rolling_hantang?'
      // if (this.tableRow.raffledType === "guest") {
      //   path = '/lottery/prizeRolling?'
      // }
