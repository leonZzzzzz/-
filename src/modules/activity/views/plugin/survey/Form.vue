<template>
  <div style="background:#fff;" ref="wrap" v-loading="dataLoading">
    <div class="tabbar">
      <div
        class="item"
        ...
        <div class="setting">
          ...
          <el-form :model="model" ref="model" label-position="top" size="small">
            ...
            <el-form-item label="封面图" prop="picture" :rules="[{ required: true, message: '该字段不能为空' }]">
              <QcImageUpload v-model="model.picture" width="120px" height="120px"></QcImageUpload>
            </el-form-item>
            <el-form-item label="简介" prop="content" :rules="[{ required: true, message: '该字段不能为空' }]">
          ...
          <draggable v-model="base.model" @update="datadragEnd" :options="dragOptions">
            ...
            >
              ...
              <el-form-item
                ...
                :rules="[{ required: true, message: '该字段不能为空' }]"
              >
                <el-input size="small" v-model="item.title" style="width:300px;" placeholder="请输入题目"></el-input>
                <!-- 添加图片 -->
                <el-button
    ...
    <div class="bottom-btn">
      <el-button type="primary" @click="submit()" style="margin-left: 30px;" size="small" :loading="loading"
        >保存</el-button
      >
    </div>
    <image-dialog
...
export default {
  ...
  methods: {
    ...
    detail(id) {
      this.$http.getSurvey({ id: id }).then(res => {
        console.log(res)
    ...
    detailQ(id) {
      this.$http.getSurveyQuestion({ surveyId: id }).then(res => {
        let data = res.data.data
    ...
    getTagList() {
      ...
      this.$http.listTag(data).then(res => {
