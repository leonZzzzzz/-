      ...
      <div class="speech-dialog">
        <div class="speech_dialog_content">
          ...
          <el-form label-width="120px" label-position="left">
            ...
            <el-form-item
              label="雷达追踪"
              v-if="['image_text'].includes(materialEditor.type) && !imageTextRadarSetting.isFromRadar"
            >
              <div class="radar-content">
                <el-checkbox v-model="imageTextRadarSetting.isRadar" :disabled="isImageTextDisabledFn"
                  >同步到雷达链接</el-checkbox
                >
                <el-tooltip
                  effect="dark"
                ...
                <!-- <div class="radar-input" v-if="imageTextRadarSetting.isRadar && !speechEditDialog.editForm.isRadar" style="display: flex; margin-top: 10px">
                  <span class="input-label" style="width: 80px">雷达标题</span>
                  <el-input placeholder="请输入雷达标题，必填" v-model="imageTextRadarSetting.radarTitle" maxlength="60" show-word-limit></el-input>
                </div> -->
              </div>
            </el-form-item>
            <el-form-item
            ...
            >
              <div class="radar-setting">
                <div class="setting-item">
                  <el-checkbox v-model="imageTextRadarSetting.form.isBehaviorNotice" disabled>行为通知</el-checkbox>
                  <span class="s-tips">（当客户点击雷达时，发送雷达的员工将会收到消息提醒）</span>
                </div>
                <div class="setting-item">
                  <el-checkbox v-model="imageTextRadarSetting.form.isActionNotice" disabled>动态通知</el-checkbox>
                  <span class="s-tips">（当客户点击雷达时，会将客户的打开行为记录在客户动态里）</span>
                </div>
                <div class="setting-item">
                  <el-checkbox v-model="imageTextRadarSetting.form.isBehaviorTag" disabled>客户标签</el-checkbox>
                ...
                <div class="setting-tags" v-if="imageTextRadarSetting.form.isBehaviorTag">
                  <el-tag
                    type="info"
                    class="tag-item"
                    v-for="(item, index) in imageTextRadarSetting.form.behaviorTagList"
                    :key="index"
                    >{{ item.name }}</el-tag
                  >
                </div>
            ...
            <el-form-item
              label="雷达追踪"
              v-if="['file'].includes(materialEditor.type) && !fileRadarSetting.isFromRadar"
            >
              <div class="radar-content">
                <el-checkbox v-model="fileRadarSetting.isRadar" :disabled="isFileDisabledFn"
                  >同步到雷达链接</el-checkbox
                >
                <el-tooltip
                  effect="dark"
                ...
                <!-- <div class="radar-input" v-if="fileRadarSetting.isRadar && !speechEditDialog.editForm.isRadar" style="display: flex; margin-top: 10px">
                  <span class="input-label" style="width: 80px">雷达标题</span>
                  <el-input placeholder="请输入雷达标题，必填" v-model="fileRadarSetting.radarTitle" maxlength="60" show-word-limit></el-input>
                </div> -->
              </div>
            </el-form-item>
            <el-form-item
            ...
            >
              <div class="radar-setting">
                <div class="setting-item">
                  <el-checkbox v-model="fileRadarSetting.form.isBehaviorNotice" disabled>行为通知</el-checkbox>
                  <span class="s-tips">（当客户点击雷达时，发送雷达的员工将会收到消息提醒）</span>
                </div>
                <div class="setting-item">
                  <el-checkbox v-model="fileRadarSetting.form.isActionNotice" disabled>动态通知</el-checkbox>
                  <span class="s-tips">（当客户点击雷达时，会将客户的打开行为记录在客户动态里）</span>
                </div>
                <div class="setting-item">
                  <el-checkbox v-model="fileRadarSetting.form.isBehaviorTag" disabled>客户标签</el-checkbox>
                  <span class="s-tips">（给点击雷达的客户打上选中的标签）</span>
                </div>
                <div class="setting-tags" v-if="fileRadarSetting.form.isBehaviorTag">
                  <el-tag
                    type="info"
                    class="tag-item"
                    v-for="(item, index) in fileRadarSetting.form.behaviorTagList"
                    :key="index"
                    >{{ item.name }}</el-tag
                  >
                </div>
...
export default {
  ...
  methods: {
      ...
      if (typeof data === 'object') {
        switch (this.materialEditor.type) {
          ...
          case 'video':
            ...
            break
          case 'file':
            model.input.fileName = data.fileName
            break
        }
      }
    },
        ...
        if (typeof params === 'object') {
          if (submitType === 'add') {
            if (model.input.videoName === '') {
              ...
              this.$http
                .getVideoSignatureDetail({ fileId })
                .then(res => {
      ...
      if (
        ...
        if (type === 'image_text') {
          ...
          if (that.imageTextRadarSetting.isFromRadar) {
            that.$set(params, 'radarId', that.imageTextRadarSetting.form.id)
          }
        } else if (type === 'file') {
          if (that.fileRadarSetting.isRadar) {
            // 默认文件标题
            that.fileRadarSetting.radarTitle = params.name
            // console.log('文件标题', params.name)
            // if (that.fileRadarSetting.radarTitle === '') {
            //   that.$message.warning('请输入雷达标题')
            //   that.createNewWordsWayFlag = false
            //   return
            // } else {
            //   // 文件类型的时候要多传一个radarType = file，fileName
          ...
          if (that.fileRadarSetting.isFromRadar) {
            that.$set(params, 'radarId', that.fileRadarSetting.form.id)
          }
        ...
        // 发送请求
        this.createNewWordsWayFlag = false
        that.$http.addSpeechcraft(params).then(() => {
    ...
    updateSpeechData(type, form) {
      ...
      /*
      let obj = that.speechEditDialog.editForm
      let params = {
        type: type,
        name: form.name,
        ...
        source: obj.source,
        speechcraftGroupList: obj.speechcraftGroupList,
      ...
      // list.push(obj1)
      // list.push(obj2)
      // that.$set(params, 'speechcraftGroupList', l·ist)
      // 添加行为标签
      let behaviorTagList = this.selectedBehaviorTags.map(item => {
        return { id: item.id }
      })
      ...
      */
      ...
      if (type === 'image_text') {
        ...
        if (this.imageTextRadarSetting.isFromRadar) {
          params.radarId = this.imageTextRadarSetting.form.id
        } else {
          params.radarId = ''
        }
      } else if (type === 'file') {
        if (this.fileRadarSetting.isRadar) {
          ...
          console.log('文件标题', params.name)
          // if (this.fileRadarSetting.radarTitle === '') {
          //   this.$message.warning('请输入雷达标题')
          //   this.createNewWordsWayFlag = false
          //   return
          // } else {
        ...
        if (this.fileRadarSetting.isFromRadar) {
          params.radarId = this.fileRadarSetting.form.id
        } else {
          params.radarId = ''
        }
      }
      ...
      this.createNewWordsWayFlag = false
      that.$http.updateSpeechcraft(params).then(() => {
    ...
    checkBehaviorTagActive(obj) {
      return this.behaviorGroupItemTags.some(item => item.id === obj.id)
    },
    // 选择行为标签
    onBehaviorTagSelect(obj) {
      ...
      } else if (this.selectedBehaviorTags.length >= 9) {
