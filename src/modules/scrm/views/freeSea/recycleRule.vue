...
export default {
  ...
  data() {
    return {
      ...
      model: {
        ...
        ruleInfoList: [{ lifeCycleId: '', days: 7, beforeRemindDays: 3, sortNum: '0' }], //规则列表
        ruleUserList: [], //成员列表
  ...
  methods: {
    ...
    chooseMember() {
      ...
      //   this.selectedList.push({ id: item.userId, name: item.userName || item.deptName, deptName: item.deptName })
      // })
    ...
    getDetail(id) {
      ...
      this.$http.recyclingRulesDetail({ relationId: id }).then(res => {
        ...
        console.log('传递值', this.selectedList)
        // setTimeout(() => {
        //   this.setDefaultCheckedKeys()
        // }, 500);
      })
    ...
    // 添加规则
    addRules() {
      this.model.ruleInfoList.push({ lifeCycleId: '', days: 7, beforeRemindDays: 3, sortNum: '0' })
    },
    ...
    innerclose(flag) {
      console.log('返回值', flag)
      this.selectedList = flag
      ...
      this.model.ruleUserList = []
      flag.forEach(item => {
        this.model.ruleUserList.push({
          userId: item.id,
          userName: item.name,
          type: item.type
        })
      })
      console.log(this.model.ruleUserList)
    },
    elCascaderOnlick(e) {
      let that = this
      setTimeout(function () {
