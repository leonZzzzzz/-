  ...
  data() {
    ...
    return {
      ...
      model: {
        ...
        // 有效天数
        referenceIdList: '',
        // 范围对象id，如果couponScope==101可不填，couponScope=204则填对应店铺id, 多个id使用英文逗号拼接
        visibility: 'public',
        // 可见性：public=无限制，vip-only=vip专享
        showInList: true,
      ...
      formVisible: false,
      option: {
        url: 'api/admin/v1/coupon_rule/list?ruleType=1'
      ...
      menu: [
        {
          ...
          label: '新建优惠券',
          on: () => {
            this.$router.push('/main/mall/coupon/form')
          }
        }
      ],
      column: [
            ...
            return (
              <div>
                ...
                {['waiting', 'ongoing'].includes(row.status) && (
                  ...
                  <el-button
                    type="text"
                    onClick={() => this.$router.push(`/main/mall/coupon/form?id=${row.id}&status=${row.status}`)}
                  >
                    修改
                  </el-button>
                )}
