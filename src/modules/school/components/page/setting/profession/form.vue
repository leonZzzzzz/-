<template>
  <el-dialog :title='id ? "修改" : "添加"' :visible.sync="visible" width="880px" :append-to-body="true" :before-close="close" :close-on-click-modal="false" :close-on-press-escape="false">
    <div style="overflow: auto;" v-loading="dataLoading">
      <el-form :model="model" ref="model" class="new-form" label-position="top">
        <el-form-item class="item-text">
          <el-form-item label="名称" prop="name" :rules="[{ required: true, message: '该字段不能为空'}]">
            <el-input size="small" v-model="model.name" clearable maxlength="32" placeholder="最多可输入32位"></el-input>
          </el-form-item>
          <el-form-item label="排序">
            <el-input-number size="small" v-model="model.seqNum"></el-input-number>
          </el-form-item>
          <!-- <el-form-item label="类型">
            <el-radio-group v-model="model.type" :disabled="id !== ''">
              <el-radio v-for="(item, i) in typeList" :key="i" :label="item.index" @change="changeRadio">{{item.name}}</el-radio>
            </el-radio-group>
          </el-form-item> -->
        </el-form-item>
        <!-- <el-form-item v-if="model.type > 1 && !id">
          <el-form-item label="学位">
            <el-select v-model="list1Id" @change="changeSelect">
              <el-option v-for="(item, i) in list1" :key="i" :label="item.name" :value="item.id" ></el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="系别" v-if="list1Id && this.model.type > 2">
            <el-select v-model="list2Id" @change="changeSelect">
              <el-option v-for="(item, i) in list2" :key="i" :label="item.name" :value="item.id" ></el-option>
            </el-select>
          </el-form-item>
        </el-form-item> -->
      </el-form>
    </div>
    <span slot="footer" class="dialog-footer">
      <el-button size="small" @click="close(false)">取 消</el-button>
      <el-button size="small" type="primary" @click="success()" :loading="loading">确 定</el-button>
    </span>
  </el-dialog>
</template>

<script>
import api from "apiSchool/common";

export default {
  props: {
    visible: Boolean,
    id: {
      type: String,
      default: ""
    },
    type: Number,
    parentId: String,
  },
  data() {
    return {
      model: {
        name: "",
        type: '',
        seqNum: 0,
        parentId: ""
      },
      typeList: [
        { name : '学位', index: 1 },
        { name : '系别', index: 2 },
        // { name : '专业', index: 3 },
      ],
      loading: false,
      dataLoading: false,
      list1: [],
      list1Id: '',
      list2: [],
      list2Id: '',
    };
  },
  watch: {
    visible(val) {
      if (val) {
        if (this.id != "") {
          this.dataLoading = true;
          this.detail();
        } else {
          this.model.type = this.type
          if (this.type === 2) this.model.parentId = this.parentId
        }
        console.log(this.model);
      }
    }
  },
  methods: {
    //获取
    async detail() {
      let res = await api.professionGet({id: this.id})
      this.model = res.data.data;
      this.dataLoading = false;
    },   
    changeRadio(i) {
      this.list1 = []
      this.list1State = false
      if (i > 1) {
        let model = {
          type: i - 1
        }
        this.professionList({type: 1}, 1)
      }
    },
    changeSelect(parentId) {
      this.professionList({type: this.list2Id ? 3 : this.list1Id ? 2 : '', parentId}, 2) 
    },
    professionList(model, val) {
      api.professionList(model).then(res => {
        console.log(res.data);
        this[`list${model.type}`] = res.data.data.list
      })
    },
    success() {
      console.log(this.model);
      this.$refs.model.validate(valid => {
        if (valid) {
          this.loading = true;
          this.saveModel(this.model, this.id ? 1 : 0)
        } else {
          return false;
        }
      });
    },
    async saveModel(model, val) {
      try {
        let type = val ? 'professionUpdate' : 'professionInsert'
        let res = await api[type](model)
        this.loading = false;
        this.$message.success(val ? '更新成功' : '添加成功')
        this.close(true);
      } catch(e) {
        console.log(e);
        this.loading = false;
      }
    },
    close(flag) {
      this.reduc();
      this.$emit("close", flag, this.type);
    },
    reduc() {
      this.model = {
        name: "",
        type: '',
        seqNum: 0,
        parentId: ""
      }
      this.list1 = []
      this.list1Id = ''
      this.list2 = []
      this.list2Id = ''
    },
  }
};
</script>

