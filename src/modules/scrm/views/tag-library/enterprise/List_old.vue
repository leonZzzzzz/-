  ...
  <div class="enterprise-tag">
    <div class="container">
      <!-- <div class="query-table-box">
        <div class="search-content">
          <div class="search-input">
            <el-input size="small" placeholder="请输入标签名称、标签组名称" v-model="searchForm.content"></el-input>
          </div>
      ...
      <div class="table-info">
        <div class="info-left">
          <!-- <div class="info-msg">目前已有标签组：&nbsp;<span>0</span></div> -->
          <div class="info-msg">
            目前已有企业标签：&nbsp;<span>{{ page.total }}</span>
          </div>
        </div>
    ...
    >
      ...
      >
        <el-form-item label="标签组名称:">
          <span class="addtag-item-span">{{ models.groupName }}</span>
        </el-form-item>
            ...
            >
              ...
              <el-col :span="2">
                <i
                  ...
                  @click="deteleEditRow(item.tagId, item.id)"
                ></i>
              </el-col>
            </el-row>
          </div>
...
export default {
  ...
  methods: {
    ...
    getTagPage() {
      let that = this
      that.$http.getEnterpriseTagPage(that.page).then(res => {
    ...
    refreshTag() {
      let that = this
      that.$http.refreshEnterpriseTag().then(() => {
        that.$message.success('标签同步成功')
        // 刷新页面
        that.page.pageNum = 1
    ...
    showEditDialog(row) {
      ...
      this.models.groupName = row.name
      this.models.groupId = row.groupId
      this.models.order = row.order
      row.tagList.forEach(item => {
        this.models.tagList.push({ name: item.name, order: item.order, tagId: item.tagId })
      })
    ...
    sumbitEditDialog() {
      ...
      let flags = false
      let newTagList = []
      if (this.models.groupName == '') {
        this.$message.warning('请输入标签分组名称')
      } else if (this.models.tagList[0].name == '') {
        ...
        if (flags == true) {
          ...
          this.$http.updateEnterpriseGroup(this.models).then(() => {
    ...
    deteleTagGroup() {
      ...
      this.$confirm('是否删除该数据?', '提示', {
        ...
        .then(() => {
          wxTagGroupIdList.push(this.models.groupId)
    ...
    sumbitAddDialog() {
      ...
      if (this.model.groupName == '') {
        this.$message.warning('请输入标签分组名称')
      } else if (this.model.tagList[0].name == '') {
        this.$message.warning('请输入标签名')
      } else {
        let newArr = []
