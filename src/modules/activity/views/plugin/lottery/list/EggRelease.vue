<template>
  <QcNavigationPage content="砸金蛋设置" v-loading="dataLoading" style="min-width:1000px">
    ...
    <div class="context-box" :style="{ height: `${areaHeight}px` }" ref="context">
      <el-form class="form" :model="model" ref="base" label-position="left" label-width="100px" size="small">
        ...
        <!-- 不需要人 -->
        <div v-if="false">
          <el-form-item label="参与嘉宾" prop="sourceType" :rules="[{ required: false, message: '该字段不能为空' }]">
        ...
        <table class="property-table item_error">
          ...
          <tr v-for="(item, i) in model.itemList" :key="i">
            ...
            <td>
              ...
              >
                <el-input-number
                  :min="item.awardType === 'lose' ? 0 : 1"
                  :max="item.awardType === 'lose' ? 0 : 9999"
                  size="small"
            ...
            <td>
              <el-form-item
                ...
                :rules="[{ required: true, message: '该字段不能为空' }]"
              >
                <el-input size="small" v-model.number="item.rate" style="width: 125px" placeholder="0~100">
        ...
        <el-form :inline="true" label-position="top">
          <el-form-item label="活动标题" prop="bgImgUrl" style="margin-right: 20px;">
          ...
          <el-form-item label="背景图片" prop="bgImgUrl" style="margin-right: 20px;">
          ...
          <!-- <el-form-item v-if="model.sourceType === 1 || model.sourceType === 2" label="活动二维码" prop="qrCodeUrl" :rules="[{ required: false, message: '该字段不能为空'}]">
        ...
        <FormTypeTitle text="音乐设置"></FormTypeTitle>
        <el-form :inline="true" label-position="top">
          <el-form-item label="背景音乐(mp3)" prop="bgMusic" style="margin-right: 20px;">
            <up-load-file
              id="bgMusic"
              title="上传"
              :fileType="['audio/mp3']"
              ...
              @file-success="bgMusicSuccess"
            ></up-load-file>
            <audio
              v-if="model.bgMusic"
              :src="imgHost + model.bgMusic"
              controls="controls"
              style="display:block;outline: none;margin-top: 5px;width: 250px;height: 30px;"
            ></audio>
          </el-form-item>
          <el-form-item label="中奖音乐(mp3)" prop="winMusic" style="margin-right: 20px;">
            <up-load-file
              id="winMusic"
              title="上传"
              :fileType="['audio/mp3']"
              accept=".mp3"
              url="api/v1/attachments/images/tencent_cloud"
              @file-success="winMusicSuccess"
            ></up-load-file>
            <audio
              v-if="model.winMusic"
              :src="imgHost + model.winMusic"
              controls="controls"
              style="display:block;outline: none;margin-top: 5px;width: 250px;height: 30px;"
            ></audio>
          </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async detail(id) {
      ...
      let res = await this.$http.getLottery({ id: id })
    ...
    awardTypeChange(item) {
      if (item.awardType === 'lose') {
        item.awardNum = 0
      }
    },
    addItem() {
    ...
    bgMusicSuccess(val) {
      this.model.bgMusic = val
    },
    winMusicSuccess(val) {
      this.model.winMusic = val
    }
  }
}
</script>
