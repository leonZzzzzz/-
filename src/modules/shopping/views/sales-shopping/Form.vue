<template>
  <QcNavigationPage content="拼团信息" style="width:90%;margin:0 auto;">
...
export default {
  ...
  methods: {
    getProduct(id) {
      this.$http.getProduct({ id }).then((res) => {
        res.data.data.productItems.map((item) => {
          ...
          item.id = ''
          // item.salesShoppingId = null
          item.sharerCommission = null
          item.sharerPoint = null
          item.distributerCommission = null
          item.distributerPoint = null
    ...
    getSalesShopping() {
      this.$http.getSalesShopping({ id: this.$route.query.id }).then((res) => {
        this.model = res.data.data
        this.model.items.map((item) => {
          item.sharerPoint /= 100
          item.sharerCommission /= 100
          item.distributerCommission /= 100
          item.distributerPoint /= 100
          return item
        })
    ...
    addSalesShopping(model) {
      this.$http.addSalesShopping(model).then(() => {
    ...
    updateSalesShopping(model) {
      this.$http.updateSalesShopping(model).then(() => {
        this.$message.success('保存成功，2秒后将进行跳转')
        setTimeout(() => {
          this.$router.back()
        }, 2000)
      ...
      this.$refs.model.validate((value) => {
        if (value) {
          ...
          model.items.map((item) => {
            item.sharerPoint = item.price * 100
            item.sharerCommission *= 100
            item.distributerCommission *= 100
            item.distributerPoint *= 100
            return item
          ...
          this.model.id ? this.updateSalesShopping(model) : this.addSalesShopping(model)
        }
      })
    }
  }
