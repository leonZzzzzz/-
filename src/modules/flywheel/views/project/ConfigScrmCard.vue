    ...
    >
      ...
      <div class="app-project__item-wrap">
        ...
        >
          ...
          >
            <p style="font-size: 22px;margin-top: -12px;">{{ auditModel.userVersion }}</p>
          </el-form-item>
        </el-form>
        <el-form
          style="flex: 500px 0 1;"
          label-position="left"
          label-width="85px"
        >
          <el-form-item
          ...
          <el-form-item
            label="简介"
            style="margin-bottom:0"
          >{{ auditModel.userDesc }}</el-form-item>
          <el-form-item
            label="状态"
            style="margin-bottom:0"
          >{{ auditModel.status | projectStatus }}</el-form-item>
          <el-form-item
            label="拒绝原因"
            v-if="auditModel.status === 'AUDIT_FAIL'"
            style="margin-bottom:0"
          >
        ...
        <div style="flex: 1;text-align:right">
          <template v-if="
            ...
            <el-button
              type="text"
              @click="getLatestAuditstatus(auditModel.status)"
            >查询审核状态</el-button><br />
            <el-button
              type="text"
              @click="undoCodeAudit()"
            ...
            <el-button
              type="text"
              @click="speedAudit()"
              v-if="auditModel.status === 'AUDIT_GOING'"
            >加速审核</el-button><br />
          </template>
          <template v-else>
            <el-button
      ...
      <div class="app-project__item-wrap">
        ...
        >
          <el-form-item
            ...
            style="margin-bottom:0"
          >
            <p style="font-size: 22px;margin-top: -12px;">{{ previewModel.userVersion }}</p>
          </el-form-item>
        </el-form>
        <el-form
          style="flex: 500px 0 1;"
...
import createExtJson from './scrmCardExt'
  ...
  methods: {
    ...
    async commitCode(id, templateId) {
      ...
      try {
        ...
        console.log(err)
        loading.close()
        if (!err.data) this.$message.error('生成模板失败')
      }
    },
    submitAudit() {
      if (this.auditModel) {
